    ...
    <card style="margin-top:15px;margin-bottom:30px;">
      <form method="POST" @submit.prevent="">
        ...
        <div class="col-12 float-left text-center" v-if="configureShow()">
          ...
          <button v-if="!loading" class="btn btn-sm btn-success" @click="submitForm()">Submit Reservation</button>
        ...
        <div class="col-12 float-left text-center" style="color:red;" v-else>
          Submit button will appear when everything is filled in.
        </div>
      </form>
...
methods.submitForm = function () {
  if (this.configureShow()) {
    ...
    axios.post('/api/addReservation', postData).then(response => {
      if (response.status === 201 || response.status === 200) {
        ...
        Swal.fire({
          ...
          icon: 'success',
          title: 'Success!',
          text: 'You have successfully entered a reservation.',
          showConfirmButton: false,
          timer: 3000
        })
        this.loading = false
        this.clearForm()
        // window.location.replace("https://rvselfpark.com/paymentconfirmation");
      }
    }).catch(error => {
        Swal.fire({
          icon: 'warning',
          title: 'Error!',
          text: 'It looks like an error occurred when trying to process this reservation. Please contact ac_080@outlook.com if the problem keeps happening.'
        })
    })
  }
}
...
methods.configureShow = function () {
  var arr = ['first_name', 'last_name', 'address', 'city', 'state', 'zip_code', 'phone', 'email', 'date_in', 'site_selected', 'vehicle_license', 'hear_about_us']
  var ready = true
  var that = this
...
export default {
  ...
  watch: {
    ...
    'form.length_of_stay': function () {
      if (this.form.arrival_time !== '' && this.form.date_in !== '') {
        this.checkReservations()
        this.form.site_selected = ''
      }
    },
  ...
  data () {
    return {
      credentials: {
        ...
        production: 'booking@rvselfpark.com'
      },
      loading: false,
      arrival_time_temp: '',
      form: {
